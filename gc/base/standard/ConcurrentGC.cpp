...
MM_ConcurrentGC::calculateTraceSize(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription)
{
	...
	if (allocDescription->isNurseryAllocation()) {
		remainingFree = potentialFreeSpace(env, allocDescription, currentOldFree());
	} else
#endif /* OMR_GC_MODRON_SCAVENGER */
	{
...
#if defined(OMR_GC_MODRON_SCAVENGER)
uintptr_t
MM_ConcurrentGC::currentOldFree()
{
 ...
 */
uintptr_t
MM_ConcurrentGC::potentialFreeSpace(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription, uintptr_t oldFree)
{
	OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
	...
	if (NULL != largeAllocateStats) {
		...
		if (oldFree > fragmentation) {
			oldFree -= fragmentation;
		} else {
			oldFree = 0;
		}
	}
	...
	if (scavengerStats->_nextScavengeWillPercolate) {
		...
		scavengesRemaining = (uintptr_t)(oldFree / nurseryPromotion);
	}
	...
	if ((0 == (rand() % 30)) || ((CONCURRENT_OFF != _stats.getExecutionMode()) && (0 == (rand() % 10)))) {
		omrtty_printf("potentialFree %zu nurseryInitialFree %zu scavengesRemaining %zu nurseryPromotion %zu\n", potentialFree, nurseryInitialFree, scavengesRemaining, nurseryPromotion);
	}
