...
MM_ConcurrentGCIncrementalUpdate::doConcurrentTrace(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription, uintptr_t sizeToTrace, MM_MemorySubSpace *subspace, bool threadAtSafePoint)
{
	...
	// free memory (from nursery perspective)
	// tenure-free-memory / tenuring-promotion rate * nursery-allocation-rate
	// if rate expressed per units in time or
	// tenure-free-memory / tenuring-promotion * nursery-size       <<<<<
	// if promotion is byte per a scavenge
...
#if defined(OMR_GC_MODRON_SCAVENGER)
	if (_extensions->scavengerEnabled) {
		/* Recalculate free memory at the time of kickoff with the current nursery sizing and tenure promotion */
		uintptr_t remainingFreeAtKickoff = potentialFreeSpace(env, allocDescription, _stats.getRemainingTenureFree(), _stats.getRemainingNurseryFree());

		if (remainingFreeAtKickoff >= _stats.getKickoffThreshold()) {
			adjustedCardCleaningThreshold += (remainingFreeAtKickoff - _stats.getKickoffThreshold());
		}

		omrtty_printf("doConcurrentTrace mode %zu current remainingFree %zu remaining free at kickoff %zu adjusted %zu adjustedCardCleaningThreshold %zu initialCardCleaningThreshold %zu getKickoffThreshold %zu\n",
				_stats.getExecutionMode(), remainingFree, _stats.getRemainingFree(), remainingFreeAtKickoff, adjustedCardCleaningThreshold, initialCardCleaningThreshold, _stats.getKickoffThreshold());
	}
#endif /* OMR_GC_MODRON_SCAVENGER */
