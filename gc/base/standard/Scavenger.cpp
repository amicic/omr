	...
	if (_extensions->concurrentScavengeExhaustiveTermination && isCurrentPhaseConcurrent() && !_scavengeCacheFreeList.areAllCachesReturned()) {
		...
		if (!checkAndSetShouldYieldFlag(env)) {
			if (0 == _cachedEntryCount) {
				//Assert_MM_true(!_scavengeCacheFreeList.areAllCachesReturned());
				OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
				omrtty_printf("shouldDoFinalNotify envID unreturned caches %zu\n", env->getEnvironmentId(), _scavengeCacheFreeList._totalAllocatedEntryCount - _scavengeCacheFreeList.getApproximateEntryCount());
...
MM_Scavenger::payAllocationTax(MM_EnvironmentBase *envBase, MM_MemorySubSpace *subspace, MM_MemorySubSpace *baseSubSpace, MM_AllocateDescription *allocDescription)
{
	...
	if (isCurrentPhaseConcurrent()) {
		 ...
		 */
		//if ((0.0f + flipBytesRatio) < usedMemoryRatio) {

		uint64_t totalScanTime = 0;
		uintptr_t iteration = 0;
		bool workDone = false;
		while (((-0.2f + flipBytesRatio) < usedMemoryRatio) && (totalScanTime < 3000)) {
			iteration += 1;
...
//						scanCache->cacheTop);
				...
				//if (0 == (rand() % 25)) {
//					omrtty_printf("MM_Scavenger::payAllocationTax envID %zu iteration %zu free/total mem %zu/%zu used bytes %.3f flipped global %zu bytes %zu%% of total mem; %.3f of average expected flipped %zu duration %zuus total scan %zuus\n",
//							env->getEnvironmentId(),
//							iteration,
//							freeMemory,
//							totalMemory,
...
//							totalScanTime);
		...
		if (workDone) {
			...
			omrtty_printf("MM_Scavenger::payAllocationTax envID %zu iteration %zu free/total mem %zu/%zu used bytes %.3f flipped global %zu bytes %zu%% of total mem; %.3f of average expected flipped %zu total scan %zuus\n",
					env->getEnvironmentId(),
					iteration,
					freeMemory,
					totalMemory,
