...
//				omrtty_printf("reserveInSemiSpace vmThread %llx activating\n", env->getLanguageVMThread());
				...
				if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_inactiveSurvivorCopyScanCache, (uintptr_t)cache, (uintptr_t)NULL)) {
					...
					cache = (MM_CopyScanCacheStandard *)env->_inactiveDeferredCopyCache;
					if (NULL != cache) {
						if (0 == rand() % 10) {
							omrthread_sleep(1);
						}
//						omrtty_printf("reserveInSemiSpace vmThread %llx activating deferred (1) active<-inactive %llx <- %llx\n",
...
//							omrtty_printf("reserveInSemiSpace vmThread %llx failed to activate deferred %llx\n",
...
//					omrtty_printf("reserveInSemiSpace vmThread %llx activating deferred (2) active<-inactive %llx <- %llx\n",
//							env->getLanguageVMThread(), env->_deferredCopyCache, cache);
					Assert_MM_true(NULL == env->_deferredCopyCache);
					if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_inactiveDeferredCopyCache, (uintptr_t)cache, (uintptr_t)NULL)) {
						env->_deferredCopyCache = cache;
...
//						omrtty_printf("reserveInTenure vmThread %llx activating deferred (1) active<-inactive %llx <- %llx\n",
//								env->getLanguageVMThread(), env->_deferredCopyCache, cache);
						Assert_MM_true(NULL == env->_deferredCopyCache);
						if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_inactiveDeferredCopyCache, (uintptr_t)cache, (uintptr_t)NULL)) {
							env->_deferredCopyCache = cache;
						} else {
//							omrtty_printf("reserveInTenure vmThread %llx failed to activate deferred\n", env->getLanguageVMThread());
						}
					}
					env->forceOutOfLineVMAccess();
					goto retry;
...
//					omrtty_printf("reserveInTenure vmThread %llx activating deferred (2) active<-inactive %llx <- %llx\n",
//							env->getLanguageVMThread(), env->_deferredCopyCache, cache);
					Assert_MM_true(NULL == env->_deferredCopyCache);
					if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_inactiveDeferredCopyCache, (uintptr_t)cache, (uintptr_t)NULL)) {
						env->_deferredCopyCache = cache;
...
//						omrthread_monitor_wait(_scanCacheMonitor);
						intptr_t wait_result = omrthread_monitor_wait_timed(_scanCacheMonitor, 10000, 0);
...
void
MM_Scavenger::threadReleaseCaches(MM_EnvironmentBase *envBase, bool final, bool forWalk)
{
	MM_EnvironmentStandard *env = MM_EnvironmentStandard::getEnvironment(envBase);
    J9VMThread *currentVMThread = ((J9JavaVM *)env->getLanguageVM())->internalVMFunctions->currentVMThread((J9JavaVM *)(env->getLanguageVM()));
    MM_EnvironmentStandard *currentEnv = MM_EnvironmentStandard::getEnvironment(currentVMThread->omrVMThread);

    Assert_MM_true(final >= forWalk);
	...
	if (isConcurrentInProgress()) {
		...
		if (NULL != cache) {
			...
			if (final || (_waitingCount > 0)) {
				/* racing with mutators trying to activate them */
				if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_inactiveSurvivorCopyScanCache, (uintptr_t)cache, (uintptr_t)NULL)) {
					Assert_MM_true(cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY);
					cache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_COPY;

					OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
					omrtty_printf("threadReleaseCaches vmThread %llx/%llx flags %llx inNative %zu pushing inactive survivor scan size %zu %llx-%llx-%llx-%llx forWalk %zu final %zu exclusive by %llx\n",
							env->getLanguageVMThread(),
							currentEnv->getLanguageVMThread(),
							((J9VMThread *)env->getLanguageVMThread())->publicFlags,
							((J9VMThread *)env->getLanguageVMThread())->inNative,
							(uintptr_t)cache->cacheAlloc - (uintptr_t)cache->scanCurrent,
							cache->cacheBase,
							cache->scanCurrent,
							cache->cacheAlloc,
		...
		#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
					if (forWalk) {
						/* Even though currentEnv != env, at this point it should be safe to use target thread env,
						 * since at the point we are sure that mutator thread does not have VM access and we won't race it
						 */
						clearCache(env, cache);
						// is this really necessary here, since we'll do it at the end?
						// furthermore, it's not consistant with tenure remainder which is not abandoned!?
						abandonSurvivorTLHRemainder(env);
					} else {
		...
		cache = env->_survivorCopyScanCache;
		if (NULL != cache) {
			if (currentVMThread != env->getLanguageVMThread() && !forWalk && !env->inNative()) {
				OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
				omrtty_printf("threadReleaseCaches GC vmThread %llx mutator vmThread %llx not safe to release active survivor cache\n",
						currentVMThread, env->getLanguageVMThread());
			} else  {
				/* race between:
				 * a GC thread thinking this is inactive thread wanting to deactivate cache and
				 * the owner mutator thread trying to flush it
				 */
				// todo: same for tenure (and likely deferred)
				if (0 == rand() % 10) {
					omrthread_sleep(1);
				}
//				if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_survivorCopyScanCache, (uintptr_t)cache, (uintptr_t)NULL)) {
					env->_survivorCopyScanCache = NULL;
					Assert_MM_true(cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY);
					// to do: we may override final being false set by the caller, if we know that we just entered the final phase
//					if (final || ((NULL != _dispatcher->_task) && (_dispatcher->_task->getThreadCount() == _waitingCount))) {
					if (final || (_waitingCount > 0)) {
						cache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_COPY;
			#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
						env->_scavengerStats._releaseScanListCount += 1;
						...
						clearCache(env, cache);
						OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
						omrtty_printf("threadReleaseCaches vmThread %llx pushing active survivor scan size %zu %llx-%llx-%llx-%llx final %zu forWalk %zu\n",
							env->getLanguageVMThread(),
							(uintptr_t)cache->cacheAlloc - (uintptr_t)cache->scanCurrent,
							cache->cacheBase,
							cache->scanCurrent,
						...
						omrtty_printf("threadReleaseCaches vmThread %llx deactivating survivor cache %llx<-%llx\n",
								env->getLanguageVMThread(), env->_inactiveSurvivorCopyScanCache, cache);
						Assert_MM_true(NULL == env->_inactiveSurvivorCopyScanCache);
						env->_inactiveSurvivorCopyScanCache = cache;
					}
//				} else {
//	//				OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
//	//				omrtty_printf("threadReleaseCaches vmThread %llx failed to push or deactivate survivor cache\n", env->getLanguageVMThread());
//				}
			}
		}

		cache = (MM_CopyScanCacheStandard *)env->_inactiveTenureCopyScanCache;
		if (NULL != cache) {
			if (0 == rand() % 10) {
				omrthread_sleep(1);
			}
			if (final || (_waitingCount > 0)) {
				/* racing with mutators trying to active them */
				if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_inactiveTenureCopyScanCache, (uintptr_t)cache, (uintptr_t)NULL)) {
					Assert_MM_true(cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY);
					cache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_COPY;

//					OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
//					omrtty_printf("threadReleaseCaches vmThread %llx/%llx flags %llx pushing inactive tenure scan size %zu %llx-%llx-%llx-%llx forWalk %zu final %zu exclusive by %llx\n",
//							env->getLanguageVMThread(),
//							currentEnv->getLanguageVMThread(),
//							((J9VMThread *)env->getLanguageVMThread())->publicFlags,
//							(uintptr_t)cache->cacheAlloc - (uintptr_t)cache->scanCurrent,
...
//							(_extensions->gcExclusiveAccessThreadId ? _extensions->gcExclusiveAccessThreadId->_language_vmthread : NULL));
		...
		#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
					if (forWalk) {
						// even though this is currentEnv != env, it should be safe to use env, since at the point we have already Xaccess and we don't race anyone
						clearCache(env, cache);
//						if (final) {
//							abandonTenureTLHRemainder(env, true);
//						}
...
//					omrtty_printf("threadReleaseCaches vmThread %llx failed to push inactive tenure cache\n", env->getLanguageVMThread());
		...
		if (NULL != cache) {
			if (currentVMThread != env->getLanguageVMThread() && !forWalk && !env->inNative()) {
//				OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
//				omrtty_printf("threadReleaseCaches GC vmThread %llx mutator vmThread %llx not safe to release active tenure cache\n",
//						currentVMThread, env->getLanguageVMThread());
			} else  {
				if (0 == rand() % 10) {
					omrthread_sleep(1);
				}
//				if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_tenureCopyScanCache, (uintptr_t)cache, (uintptr_t)NULL)) {
					env->_tenureCopyScanCache = NULL;
					Assert_MM_true(cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY);
	//				if (final || ((NULL != _dispatcher->_task) && (_dispatcher->_task->getThreadCount() == _waitingCount))) {
					if (final || (_waitingCount > 0)) {
...
//								env->getLanguageVMThread(), env->_inactiveTenureCopyScanCache, cache);
						Assert_MM_true(NULL == env->_inactiveTenureCopyScanCache);
						env->_inactiveTenureCopyScanCache = cache;
					}
//				} else {
//	//				OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
//	//				omrtty_printf("threadReleaseCaches vmThread %llx failed to push or deactivate tenure cache\n", env->getLanguageVMThread());
//				}
			}
		}
		...
		cache = env->_deferredCopyCache;
		if (NULL != cache) {
			// there should be no dangling deferred copy cache. if we just pushed survivor or tenure copy cache we should push deferred too.
			if (currentVMThread == env->getLanguageVMThread() || forWalk || env->inNative() || (NULL == env->_survivorCopyScanCache) || (NULL == env->_tenureCopyScanCache)) {
				if (0 == rand() % 10) {
					omrthread_sleep(1);
				}
				/* race between mutator thread actual owner and another mutator thread that helps with concurrent termination */
				if ((uintptr_t)NULL != MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&env->_deferredCopyCache, (uintptr_t)cache, (uintptr_t)NULL)) {
					env->_deferredCopyCache = NULL;
					if (0 == (cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY)) {
//						OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
//						omrtty_printf("threadReleaseCaches vmThread %llx/%llx active deferred cache %llx flags %llx\n",
//									env->getLanguageVMThread(), currentVMThread,
//									cache, cache->flags);
					}
					Assert_MM_true(cache->flags & OMR_SCAVENGER_CACHE_TYPE_COPY);
					Assert_MM_true(cache->flags & OMR_SCAVENGER_CACHE_TYPE_CLEARED);
	//				if (final || ((NULL != _dispatcher->_task) && (_dispatcher->_task->getThreadCount() == _waitingCount))) {
					if (final || (_waitingCount > 0) || (NULL == env->_survivorCopyScanCache) || (NULL == env->_tenureCopyScanCache)) {
						...
						omrtty_printf("threadReleaseCaches vmThread %llx deactivating deferred cache inactive<-active %llx<-%llx\n",
								env->getLanguageVMThread(), env->_inactiveDeferredCopyCache, cache);
						Assert_MM_true(NULL == env->_inactiveDeferredCopyCache);
						env->_inactiveDeferredCopyCache = cache;
					}
				} else {
					OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
		...
		if (forWalk) {
//			OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
//			omrtty_printf("threadReleaseCaches vmThread %llx abandon forWalk\n", env->getLanguageVMThread());
